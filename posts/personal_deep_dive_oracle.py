"""Deep Dive #6: Oracle Corporation (ORCL), auf Nutzerwunsch vom 2026-09-06
(urspruenglicher Kandidat, dann erst als Schnell-Check mit Adobe kombiniert
-- jetzt zusaetzlich der volle Deep Dive). Gleiche Struktur wie
posts/personal_deep_dive_vst.py -- Akzentfarbe Oracle Red (#C74634).

Statt eines eindeutigen Insider-Form-4 (nicht auffindbar) hier die naeher
liegende Institutionen-/Finanzierungs-Perspektive: Michael Burrys
Short-Position und die Fremdfinanzierung des KI-Ausbaus.

Aufruf:
  python posts/personal_deep_dive_oracle.py
"""
import json
import sys
...
NAME = "personal_deep_dive_oracle"
...
NAME_FULL = "Oracle"
WKN = "871460"
LOGO_PATH = ROOT / "assets" / "orcl_logo_icon.png"
PRICES_PATH = ROOT / "data" / "demo_ORCL_prices.json"
...
def slide_1_intro():
    img, draw = base_slide()
    build_header(draw, subtitle="DEEP DIVE #6")
    ...
    tag = "BEWERTUNG  ·  KURSANALYSE  ·  FINANZIERUNG"
...
def slide_2_overview():
    ...
    lines = wrap_text(draw, "Oracle war jahrzehntelang vor allem fuer Datenbank-Software "
                             "bekannt -- inzwischen einer der grossen Anbieter von KI-Cloud-"
                             "Infrastruktur, mit einem milliardenschweren Auftragsbestand u.a. "
                             "von OpenAI.", body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 33
    y += 10

    lines2 = wrap_text(draw, "Berichtet Donnerstag dieser Woche Quartalszahlen -- gilt als "
                              "eine der am meisten beachteten Bilanzen fuer die Frage, wie "
                              "tragfaehig der KI-Infrastruktur-Boom ist.", body_font,
    ...
    box_h = 66
    stats = [
        ("KURS", "158,78 USD"), ("MARKTKAP.", "457,4 Mrd. USD"), ("52W-HOCH", "345,72 USD"),
        ("52W-TIEF", "114,50 USD"), ("HAUPTSITZ", "Austin, Texas"), ("BOERSE", "NYSE"),
        ("SEGMENT", "Cloud/KI-Infrastruktur"), ("NAECHSTE ZAHLEN", "10. Sept. 2026"), ("UMSATZ (TTM)", "67,4 Mrd. USD"),
    ]
...
def slide_3_bewertung():
    ...
    draw.text((B.MARGIN_LEFT, y), "Mehr als halbiert seit dem Jahreshoch --", font=font(B.SANS_BOLD, 27), fill=CREAM)
    y += 38
    draw.text((B.MARGIN_LEFT, y), "trotzdem noch kein Schnaeppchen-KGV.", font=font(B.SANS_BOLD, 27), fill=CREAM)
    y += 62
    ...
    box_h = 78
    stats = [
        ("KGV (TRAILING)", "27,2x"), ("KGV (FORWARD)", "19,7x"),
        ("MARKTKAP.", "457,4 Mrd. USD"), ("UMSATZ-WACHSTUM", "+17,3%"),
    ]
    for i, (label, value) in enumerate(stats):
        row, col = divmod(i, cols)
        bx = B.MARGIN_LEFT + col * (box_w + gap)
    ...
    lines = wrap_text(draw, "Netto-Gewinn stieg im letzten Geschaeftsjahr um 37,3% auf 17,0 "
                             "Mrd. USD -- das Forward-KGV von 19,7x liegt trotz des starken "
                             "Kursrueckgangs weiter ueber dem breiten Marktdurchschnitt, "
                             "getrieben von den Wachstumserwartungen im Cloud-Geschaeft.",
...
def slide_5_kursanalyse():
    ...
    draw.text((B.MARGIN_LEFT, y), "Vom Rekordhoch tief in die Korrektur.", font=font(B.SANS_BOLD, 30), fill=CREAM)
    y += 56

    chart_h = 400
    draw_price_chart(draw, B.MARGIN_LEFT, y, W - B.MARGIN_LEFT - B.MARGIN_RIGHT, chart_h,
                      "Kursverlauf 1 Jahr -- 52W-Hoch 345,72 USD, 52W-Tief 114,50 USD")
    y += chart_h + 28

    body_font = font(B.SANS_BOLD, 22)
    lines = wrap_text(draw, "Die Aktie steht rund 54% unter ihrem 52-Wochen-Hoch -- Zweifel "
                             "an der Finanzierbarkeit des KI-Ausbaus und Sorgen um die "
                             "Verschuldung haben den Kurs seit dem Hoch deutlich belastet, "
                             "trotz weiter wachsender Umsaetze.",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 32
...
def slide_6_finanzierung():
    img, draw = base_slide()
    y = build_header(draw)
    draw.text((B.MARGIN_LEFT, y), "FINANZIERUNG & POSITIONIERUNG", font=font(B.SANS_BOLD, 17), fill=GREEN_MID)
    y += 40

    draw.text((B.MARGIN_LEFT, y), "Der KI-Ausbau wird ueberwiegend", font=font(B.SANS_BOLD, 26), fill=CREAM)
    y += 36
    draw.text((B.MARGIN_LEFT, y), "auf Kredit finanziert.", font=font(B.SANS_BOLD, 26), fill=CREAM)
    y += 52

    items = [
        ("GESCHAEFTSJAHR 2026", "Oracle nahm 43 Mrd. USD an neuen Schulden auf plus 5 Mrd. USD ueber Aktienverkaeufe, um den Infrastruktur-Ausbau zu finanzieren -- fuer das laufende Jahr werden nochmal rund 40 Mrd. USD erwartet."),
        ("13. AUGUST 2026", "'Big Short'-Investor Michael Burry meldete laut Berichten den Ausbau einer Short-Position (Wette auf fallende Kurse) bei rund 152 USD je Aktie."),
    ]
    ...
    lines = wrap_text(draw, "Eine einzelne Short-Position ist kein Urteil ueber das ganze "
                             "Unternehmen -- trotzdem ein Datenpunkt, der zeigt, wie kontrovers "
                             "die Aktie aktuell diskutiert wird. Quellen: oeffentliche "
                             "Unternehmens- und Marktberichte.", note_font,
                       W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
...
def slide_7_fazit():
    ...
    lines = wrap_text(draw, "Oracle ist mitten in einem riesigen KI-Wette-Zyklus: hohe "
                             "Investitionen, hohe Schulden, hohe Erwartungen -- und ein "
                             "Kurs, der bereits ueber die Haelfte seines Jahreshochs verloren "
                             "hat. Die Zahlen am Donnerstag werden zeigen, ob der Markt die "
                             "Wette gerade zu pessimistisch oder zu optimistisch einpreist.",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 34
...
def main():
    slides = [slide_1_intro, slide_2_overview, slide_3_bewertung, slide_4_cta,
              slide_5_kursanalyse, slide_6_finanzierung, slide_7_fazit]
