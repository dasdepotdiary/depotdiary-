"""Deep Dive #8: Palantir Technologies (PLTR), auf Nutzerwunsch vom 2026-09-09
(Hintergrund-Auftrag, ohne weitere Rueckfrage bauen und queuen). Gleiche
Struktur wie posts/personal_deep_dive_oracle.py / personal_deep_dive_adobe.py
-- eigene Akzentfarbe (Stahlblau), passend zum Defense-/Intelligence-Thema.

Bewusste Abweichung von der Oracle/Adobe-Vorlage: keine "BEWERTUNG"-Folie mit
Guenstig/Teuer-Einordnung (KGV-Vergleich mit Sektor, "kein Schnaeppchen-KGV"
o.ae.) -- das verstoesst gegen die harte Compliance-Regel in CLAUDE.md
("Bewertungen wie guenstig/teuer/unterbewertet" sind verboten). Stattdessen
eine neutrale "KENNZAHLEN"-Folie (reine Fakten, keine Einordnung) plus eine
eigene Geschaeftsmodell-Folie (Gotham/Foundry/AIP), wie vom Auftrag verlangt.
Der "auffaellige Fakt" ist hier die Auftragslage (US-Army-TITAN-Vertrag,
3. Sept. 2026) plus Peter Thiels planmaessiger Insider-Verkauf (2. Maerz
2026) -- rein faktisch, ohne Bewertung, analog zu Oracles Burry-Short-Folie.

Aufruf:
  python posts/personal_deep_dive_palantir.py
"""
import json
import sys
...
NAME = "personal_deep_dive_palantir"
...
def slide_1_intro():
    img, draw = base_slide()
    build_header(draw, subtitle="DEEP DIVE #8")
    ...
    tag = "GESCHICHTE  ·  GESCHAEFTSMODELL  ·  KENNZAHLEN"
...
def slide_2_unternehmen():
    ...
    lines = wrap_text(draw, "Gegruendet 2003 in Palo Alto von Peter Thiel (PayPal-Mitgruender), "
                             "Alex Karp und drei weiteren -- benannt nach den 'Sehsteinen' aus "
                             "Tolkiens Der Herr der Ringe. Die erste grosse Finanzierung kam 2004 "
                             "von In-Q-Tel, dem Wagniskapital-Arm der CIA.", body_font,
    ...
    lines2 = wrap_text(draw, "Das Geschaeft: Software, die riesige, verstreute Datenmengen in "
                              "einer Oberflaeche zusammenfuehrt. Gotham fuer Geheimdienste und "
                              "Militaer, Foundry seit 2016 fuer Unternehmen wie Banken oder "
                              "Kliniken, seit 2023 zusaetzlich AIP -- das bringt Sprachmodelle "
                              "direkt mit den eigenen Unternehmensdaten zusammen, statt als "
                              "eigenstaendiger Chatbot ohne Kontext.", body_font,
                        W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines2:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 31
    ...
    lines3 = wrap_text(draw, "Seit 2020 an der Boerse, per Direktnotierung statt klassischem IPO.",
...
def slide_3_kennzahlen():
    img, draw = base_slide()
    y = build_header(draw)
    draw.text((B.MARGIN_LEFT, y), "KENNZAHLEN", font=font(B.SANS_BOLD, 17), fill=GREEN_MID)
    y += 40

    draw.text((B.MARGIN_LEFT, y), "Palantir in Zahlen --", font=font(B.SANS_BOLD, 30), fill=CREAM)
    y += 40
    draw.text((B.MARGIN_LEFT, y), "Stand Anfang September 2026.", font=font(B.SANS_BOLD, 30), fill=CREAM)
    y += 56
    ...
    box_h = 78
    stats = [
        ("KURS", "182,53 USD"), ("MARKTKAP.", "439,5 Mrd. USD"), ("52W-HOCH", "207,52 USD"),
        ("52W-TIEF", "106,37 USD"), ("MITARBEITER", "rund 4.500"), ("GEGRUENDET", "2003"),
        ("HAUPTSITZ", "Miami, Florida"), ("BOERSE", "NASDAQ"), ("UMSATZ Q2 2026", "1,94 Mrd. USD (+93%)"),
    ]
    ...
    lines = wrap_text(draw, "Rund 4.500 Beschaeftigte weltweit -- fuer ein Unternehmen dieser "
                             "Marktkapitalisierung ein kleines Team. Laut CEO Alex Karp lag der "
                             "Jahresumsatz je Mitarbeiter zuletzt bei etwa 1,5 Mio. USD.",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=MUTED)
        y += 27
...
def slide_5_kursanalyse():
    ...
    draw.text((B.MARGIN_LEFT, y), "Ein Jahr zwischen Rekordhoch", font=font(B.SANS_BOLD, 30), fill=CREAM)
    y += 38
    draw.text((B.MARGIN_LEFT, y), "und harter Korrektur.", font=font(B.SANS_BOLD, 30), fill=CREAM)
    y += 54

    chart_h = 400
    draw_price_chart(draw, B.MARGIN_LEFT, y, W - B.MARGIN_LEFT - B.MARGIN_RIGHT, chart_h,
                      "Kursverlauf 1 Jahr -- 52W-Hoch 207,52 USD, 52W-Tief 106,37 USD")
    y += chart_h + 28

    body_font = font(B.SANS_BOLD, 22)
    lines = wrap_text(draw, "Vom Rekordhoch im November 2025 fiel der Kurs bis Mitte 2026 um "
                             "zeitweise fast die Haelfte, bevor er sich wieder deutlich erholte -- "
                             "trotz durchgehend starker Quartalszahlen in diesem Zeitraum. Eine der "
                             "schwankungsreichsten grossen Tech-Aktien des Jahres.",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 32
...
def slide_6_auftragslage():
    img, draw = base_slide()
    y = build_header(draw)
    draw.text((B.MARGIN_LEFT, y), "AUFTRAGSLAGE & INSIDER", font=font(B.SANS_BOLD, 17), fill=GREEN_MID)
    y += 40

    draw.text((B.MARGIN_LEFT, y), "Neue Grossauftraege --", font=font(B.SANS_BOLD, 27), fill=CREAM)
    y += 38
    draw.text((B.MARGIN_LEFT, y), "und ein planmaessiger Insider-Verkauf.", font=font(B.SANS_BOLD, 27), fill=CREAM)
    y += 58

    items = [
        ("3. SEPTEMBER 2026", "Die US Army beauftragt Palantir mit dem Bau von acht TITAN-"
                               "Bodenstationen (KI-gestuetzte Aufklaerung) -- der naechste Schritt "
                               "nach der fruehen Auswahl von Palantirs Foundry-Plattform fuer das "
                               "Kommandosystem-Programm NGC2 der Armee."),
        ("2. MAERZ 2026", "Mitgruender Peter Thiel verkauft ueber einen vorab im November 2025 "
                           "eingereichten Rule-10b5-1-Handelsplan 2 Mio. Aktien fuer rund 280 Mio. "
                           "USD -- er haelt danach weiter etwa 3% des Unternehmens, rund 98 Mio. "
                           "Aktien."),
    ]
    body_font = font(B.SANS_BOLD, 20)
    label_font = font(B.SANS_BOLD, 18)
    ...
    lines = wrap_text(draw, "Ein einzelner, planmaessiger Insider-Verkauf ist kein Urteil ueber das "
                             "Unternehmen -- trotzdem ein Datenpunkt, den ich mir angeschaut habe. "
                             "Quellen: SEC-Meldungen, Unternehmens- und oeffentliche Berichterstattung.",
...
def slide_7_fazit():
    ...
    lines = wrap_text(draw, "In gut zwanzig Jahren vom kleinen Geheimdienst-Dienstleister mit "
                             "CIA-Anschubfinanzierung zu einem der umsatzstaerksten KI-Software-"
                             "Unternehmen der Welt -- mit zuletzt 93% Umsatzwachstum, wachsendem "
                             "Commercial-Geschaeft neben dem klassischen Regierungsgeschaeft, und "
                             "einem Aktienkurs, der 2026 zwischen Rekordhoch und harter Korrektur "
                             "pendelte.",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
...
def main():
    slides = [slide_1_intro, slide_2_unternehmen, slide_3_kennzahlen, slide_4_cta,
              slide_5_kursanalyse, slide_6_auftragslage, slide_7_fazit]
