"""Deep Dive #5: Nvidia (NVDA), auf Nutzerwunsch vom 2026-09-06.
Gleiche Struktur wie posts/personal_deep_dive_vst.py -- Akzentfarbe hier
Nvidia-Gruen (offizielle Markenfarbe #76B900).

Besonderheit: die mit Abstand groesste Marktkapitalisierung aller bisherigen
Deep Dives (5,56 Bio. USD, weltgroesstes Unternehmen) -- Zahlen aus dem
tatsaechlichen Q2-Bericht vom 26.08.2026, keine Vorschau.

Aufruf:
  python posts/personal_deep_dive_nvidia.py
"""
import json
import sys
...
NAME = "personal_deep_dive_nvidia"
...
NAME_FULL = "Nvidia"
WKN = "918422"
LOGO_PATH = ROOT / "assets" / "nvda_logo_icon.png"
PRICES_PATH = ROOT / "data" / "demo_NVDA_prices.json"
...
def slide_1_intro():
    img, draw = base_slide()
    build_header(draw, subtitle="DEEP DIVE #5")
...
def slide_2_overview():
    ...
    lines = wrap_text(draw, "Nvidia entwickelt die Chips und die Software, auf denen ein "
                             "Grossteil des aktuellen KI-Booms laeuft -- vom Training grosser "
                             "Sprachmodelle bis zu den Rechenzentren dahinter.", body_font,
    ...
    lines2 = wrap_text(draw, "Mit 5,56 Billionen USD Marktkapitalisierung aktuell das "
                              "wertvollste Unternehmen der Welt -- und mein bisher groesster "
                              "Deep Dive in dieser Reihe.", body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines2:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 33
    ...
    box_h = 66
    stats = [
        ("KURS", "230,36 USD"), ("MARKTKAP.", "5,56 Bio. USD"), ("52W-HOCH", "236,54 USD"),
        ("52W-TIEF", "164,07 USD"), ("HAUPTSITZ", "Santa Clara, CA"), ("BOERSE", "NASDAQ"),
        ("SEGMENT", "KI-Chips/Rechenzentren"), ("52W-PERFORMANCE", "+34%"), ("UMSATZ (TTM)", "303,0 Mrd. USD"),
    ]
...
def slide_3_bewertung():
    ...
    draw.text((B.MARGIN_LEFT, y), "Nahe am Rekordhoch, mit echten Gewinnen", font=font(B.SANS_BOLD, 27), fill=CREAM)
    y += 38
    draw.text((B.MARGIN_LEFT, y), "im Rueckenwind.", font=font(B.SANS_BOLD, 27), fill=CREAM)
    y += 62
    ...
    box_h = 78
    stats = [
        ("KGV (TRAILING)", "29,1x"), ("KGV (FORWARD)", "19,1x"),
        ("MARKTKAP.", "5,56 Bio. USD"), ("52W-PERFORMANCE", "+34%"),
    ]
    for i, (label, value) in enumerate(stats):
        row, col = divmod(i, cols)
        bx = B.MARGIN_LEFT + col * (box_w + gap)
    ...
    lines = wrap_text(draw, "Im letzten Quartalsbericht (26. August) meldete Nvidia 96,2 "
                             "Mrd. USD Umsatz und 2,22 USD Gewinn je Aktie -- beides deutlich "
                             "ueber den Analysten-Erwartungen von 92,2 Mrd. USD bzw. 2,09 USD. "
                             "Die Marktkapitalisierung stieg an diesem Tag von 5,07 auf 5,52 "
                             "Billionen USD.",
...
def slide_5_kursanalyse():
    ...
    draw.text((B.MARGIN_LEFT, y), "Ruhiger Aufwaertstrend statt Achterbahn.", font=font(B.SANS_BOLD, 30), fill=CREAM)
    y += 56

    chart_h = 400
    draw_price_chart(draw, B.MARGIN_LEFT, y, W - B.MARGIN_LEFT - B.MARGIN_RIGHT, chart_h,
                      "Kursverlauf 1 Jahr -- 52W-Hoch 236,54 USD, 52W-Tief 164,07 USD")
    y += chart_h + 28

    body_font = font(B.SANS_BOLD, 22)
    lines = wrap_text(draw, "Nur rund 3% unter dem 52-Wochen-Hoch -- verglichen mit den "
                             "anderen Deep Dives in dieser Reihe der ruhigste, kontinuierlichste "
                             "Kursverlauf, obwohl die Aktie mit 5,56 Billionen USD "
                             "Marktkapitalisierung die mit Abstand groesste hier ist.",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 32
...
def slide_6_insider():
    ...
    draw.text((B.MARGIN_LEFT, y), "CEO Jensen Huang hat seinen Verkaufsplan", font=font(B.SANS_BOLD, 26), fill=CREAM)
    y += 36
    draw.text((B.MARGIN_LEFT, y), "abgeschlossen.", font=font(B.SANS_BOLD, 26), fill=CREAM)
    y += 52

    items = [
        ("10B5-1-PLAN, BIS MITTE 2026", "Jensen Huang verkaufte planmaessig bis zu 6 Mio. Aktien (rund 865 Mio. USD Volumen bei Planstart) -- der Plan gilt inzwischen als im Wesentlichen abgeschlossen, insgesamt wurden mehr als 700 Mio. USD verkauft."),
        ("AKTUELLER BESTAND", "Huang haelt weiterhin rund 75,4 Mio. Aktien direkt sowie rund 786 Mio. Aktien ueber Trusts und eine Beteiligungsgesellschaft -- weiterhin der groesste Einzelaktionaer von Nvidia."),
    ]
    ...
    lines = wrap_text(draw, "Planmaessige Verkaeufe unter einem 10b5-1-Plan sagen nichts ueber "
                             "die aktuelle Einschaetzung des Managements aus -- Termine und "
                             "Mengen stehen lange im Voraus fest. Quelle: oeffentliche SEC-"
                             "Meldungen.", note_font,
                       W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
...
def slide_7_fazit():
    ...
    lines = wrap_text(draw, "Nvidia ist mit 5,56 Billionen USD das wertvollste Unternehmen "
                             "der Welt -- und liefert das Wachstum bisher tatsaechlich: Umsatz "
                             "und Gewinn im letzten Quartal deutlich ueber den Erwartungen. "
                             "Gleichzeitig kauft das Unternehmen selbst aggressiv zu (12,9 "
                             "Mrd. USD fuer Hugging Face) -- ein Zeichen dafuer, wie viel "
                             "Kapital hier bewegt wird.",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 34
