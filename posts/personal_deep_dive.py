"""Neues Format: "Deep Dive" zu einer einzelnen Aktie -- 7 Folien statt der
kompakten 1-Folie-Datenkarte (siehe personal_portfolio_update.py). Deutlich
mehr Tiefe: eigene Folie fuer Bewertung, eigene Folie fuer Kursanalyse,
eigene Folie fuer Insider-Aktivitaet, plus eine Engagement-Folie in der
Mitte (Nutzer-Wunsch 2026-09-03: "Folgt/Liked/Kommentiert"-Slide).

Rein faktisch -- KGV/KUV/Marktkap./Insider-Transaktionen sind oeffentliche
Zahlen (SEC Form 4 fuer Insider), keine eigene Bewertung ("teuer/guenstig"),
kein Kursziel, keine Kauf-/Verkaufsempfehlung (siehe CLAUDE.md-Regel). Wo
Dritt-Quellen eine Bewertung abgeben (z.B. "significantly overvalued"),
wird das NICHT uebernommen -- nur die nackte Kennzahl (KUV-Multiple) im
Vergleich zum Branchenschnitt, ohne Adjektiv.

Erste Aktie: Rocket Lab (RKLB), auf Nutzerwunsch vom 2026-09-03.

Aufruf:
  python posts/personal_deep_dive.py
"""
import json
import sys
...
TICKER = "RKLB"
NAME_FULL = "Rocket Lab"
WKN = "A2QG3G"
LOGO_PATH = ROOT / "assets" / "rklb_logo_icon.png"
PRICES_PATH = ROOT / "data" / "demo_RKLB_prices.json"

# Akzentfarbe pro Aktie an das jeweilige Firmenlogo angepasst (Nutzerwunsch
# 2026-09-03, gilt fuer jede Folge dieser Reihe) -- bei Rocket Lab das Rot
# der Rakete/Flamme aus dem Logo, statt eines fixen Gold-Tons.
ACCENT = "#E13A2E"
CHART_LINE = ACCENT
CHART_FILL = "#2A1613"
...
def slide_2_overview():
    ...
    lines = wrap_text(draw, "Rocket Lab entwickelt und startet Traegerraketen (Electron, "
                             "in Entwicklung: Neutron) und baut Satelliten-Komponenten fuer "
                             "die Raumfahrtindustrie. Gegruendet 2006 von Peter Beck, seit "
                             "2021 an der Boerse.", body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 33
    y += 10

    lines2 = wrap_text(draw, "Zwei Geschaeftsbereiche: Launch Services (Raketenstarts) und "
                              "Space Systems (Satelliten-Komponenten, Bordcomputer, Solarzellen) "
                              "-- beide wachsen zweistellig.", body_font,
    ...
    box_h = 66
    stats = [
        ("KURS", "63,81 USD"), ("MARKTKAP.", "37,6 Mrd. USD"), ("52W-HOCH", "151,00 USD"),
        ("52W-TIEF", "37,57 USD"), ("BOERSENGANG", "2021 (SPAC)"), ("HAUPTSITZ", "Long Beach, USA"),
        ("MITARBEITER", "rund 2.600"), ("NEUTRON-ERSTFLUG", "Q4 2026 (geplant)"), ("SEGMENTE", "2 (Launch/Space)"),
    ]
...
def slide_3_bewertung():
    ...
    draw.text((B.MARGIN_LEFT, y), "Hohes Wachstum, hoher Preis.", font=headline_font, fill=CREAM)
    y += 60
    ...
    box_h = 78
    stats = [
        ("KUV (PRICE/SALES)", "65,7x"), ("KUV BRANCHENSCHNITT", "3,7x"),
        ("UMSATZWACHSTUM Q2", "+52,5% YoY"), ("OPERATIVE MARGE", "-29,1%"),
        ("UMSATZ Q2", "234 Mio. USD"), ("KUV 10J-MEDIAN (EIGEN)", "21,3x"),
    ]
    for i, (label, value) in enumerate(stats):
        row, col = divmod(i, cols)
        bx = B.MARGIN_LEFT + col * (box_w + gap)
    ...
    lines = wrap_text(draw, "Das KUV liegt weit ueber dem Branchenschnitt der Luft- und "
                             "Raumfahrtindustrie -- der Markt preist also viel zukuenftiges "
                             "Wachstum ein. Auch im Vergleich zum eigenen 10-Jahres-Median "
                             "(21,3x) ist die aktuelle Bewertung deutlich hoeher. Umsatz "
                             "waechst schnell (Rekordquartal), profitabel ist das Unternehmen "
                             "noch nicht -- die operative Marge verbessert sich aber von "
                             "Quartal zu Quartal.",
...
def slide_5_kursanalyse():
    ...
    lines = wrap_text(ImageDraw.Draw(img), "Eine der volatileren Aktien in meinem Blickfeld.",
    ...
    draw_price_chart(draw, B.MARGIN_LEFT, y, W - B.MARGIN_LEFT - B.MARGIN_RIGHT, chart_h,
                      "Kursverlauf 1 Jahr -- 52W-Hoch 151,00 USD, 52W-Tief 37,57 USD")
    y += chart_h + 28

    body_font = font(B.SANS_BOLD, 22)
    lines = wrap_text(draw, "Aktuell rund 58% unter dem 52-Wochen-Hoch, aber rund 70% "
                             "ueber dem 52-Wochen-Tief -- der Kurs hat sich in diesem Jahr "
                             "mehr als vervierfacht und dann wieder deutlich korrigiert. Der "
                             "naechste grosse Kurstreiber duerfte der Erstflug der Neutron-"
                             "Rakete werden, aktuell fuer Q4 2026 geplant (kann sich noch "
                             "verschieben).",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
...
def slide_6_insider():
    ...
    headline_font = font(B.SANS_BOLD, 30)
    draw.text((B.MARGIN_LEFT, y), "Ueberwiegend Verkaeufe im Sommer 2026.", font=headline_font, fill=CREAM)
    y += 56

    items = [
        ("JUL 2026", "CEO-Familientrust (Equatorial Trust) verkauft rund 3,28 Mio. Aktien in mehreren Tranchen (6.-8. Juli)."),
        ("AUG 2026", "COO, General Counsel und VP Controller verkaufen kleinere Betraege -- ueberwiegend Sell-to-Cover fuer Steuern bei RSU-Vesting, teils planmaessige 10b5-1-Verkaeufe."),
        ("AUG 2026", "Eine Direktorin uebertraegt 261.903 Aktien als Schenkung an ihren eigenen Trust -- kein Verkauf am Markt."),
    ]
    ...
    lines = wrap_text(draw, "Keine grossen offenen Kaeufe von Insidern gefunden -- Quelle: "
...
def slide_7_fazit():
    ...
    lines = wrap_text(draw, "Rocket Lab waechst schnell -- Q2-Umsatz +52,5% -- ist dafuer "
                             "aber auch hoch bewertet (KUV 65,7x) und noch nicht profitabel. "
                             "Insider haben zuletzt ueberwiegend verkauft, meist aus "
                             "planmaessigen oder steuerlichen Gruenden. Der Kurs war in diesem "
                             "Jahr extrem volatil, und der Neutron-Erstflug (Q4 2026 geplant) "
                             "bleibt der naechste grosse Meilenstein, den ich im Blick behalte.",
                       body_font, W - B.MARGIN_LEFT - B.MARGIN_RIGHT)
    for line in lines:
        draw.text((B.MARGIN_LEFT, y), line, font=body_font, fill=CREAM)
        y += 34
